...
namespace Project.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> RegisterTeacher(TeachViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.TeacherEmail, Email = model.TeacherEmail };
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
        ...
        public async Task<ActionResult> RegisterStu(StuViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.StudentEmail, Email = model.StudentEmail };
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> RegisterParent(ParentViewModel model)
        {
            ApplicationDbContext db = new ApplicationDbContext();
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.ParentEmail, Email = model.ParentEmail };
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
